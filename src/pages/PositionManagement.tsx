...
import { Search, Filter, Plus, Users, Bot, ArrowLeft, Mail, User, MapPin, DollarSign, Clock, Building, GraduationCap } from 'lucide-react';
  ...
  const [positions, setPositions] = useState([
    {
      ...
      roleDescription: 'We are looking for a Senior Software Engineer to join our dynamic team...',
      ...
      prompt: 'Find experienced full-stack developers with strong problem-solving skills',
    ...
    },
    {
      ...
      roleDescription: 'Seeking an experienced Product Manager to drive product strategy...',
      ...
      prompt: 'Find product managers with strong analytical skills and startup experience',
...
const navigate = useNavigate();
  ...
  const calculateDaysSinceNoAction = (position:string) => {
    ...
    const hasRecentStatusUpdates = positionCandidates.some(candidate => {
      ...
      return candidate.status !== 'submitted' && candidate.status !== 'internal_screening';
    });
  ...
  const groupedPositions = groupBy === 'clientName' 
  ...
  const handleEmailCheck = () => {
    const existingCandidate = candidates.find(c => c.email === candidateEmail);
    if (existingCandidate) {
      // Tag existing candidate to position
      const updatedCandidates = [...candidates];
      const candidateIndex = updatedCandidates.findIndex(c => c.email === candidateEmail);
  ...
  const runAIResumeMatcherForPosition = (positionId) => {
    ...
    if (eligibleCandidates.length === 0) {
      alert('No candidates with "Internal Screening" status found for AI matching.');
      return;
    }

    // Simulate AI scoring (normally this would call an actual AI API)
    alert(`Running AI Resume Matcher for ${eligibleCandidates.length} candidates...`);
    ...
    setTimeout(() => {
      ...
      const topCandidates = eligibleCandidates.filter((_, index) => {
        const score = updatedCandidates.find(c => c.id === eligibleCandidates[index].id)?.aiScore || 0;
        return score >= 85;
      });
      
      alert(`AI Resume Matcher completed! ${topCandidates.length} candidates qualified for recruiter review. Others moved to "Internal Reject" status.`);
    }, 3000);
  };
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {positions.map((position) => {
              ...
              return (
                ...
                <tr key={position.id} className="hover:bg-gray-50 cursor-pointer"
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm text-gray-900">{position.clientName}</div>
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-2 w-16 bg-gray-200 rounded-full h-2">
                        <div 
                          ...
                          style={{ width: `${Math.min((position.submittedResumes / position.targetedResumes) * 100, 100)}%` }}
                        ></div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm text-gray-900">{position.ctcBudget}</div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className={`inline-flex px-3 py-1 text-sm font-medium rounded-full ${
                      daysSinceNoAction === 0 ? 'bg-green-100 text-green-800' :
                      daysSinceNoAction <= 3 ? 'bg-yellow-100 text-yellow-800' :
                      daysSinceNoAction <= 7 ? 'bg-orange-100 text-orange-800' :
                      'bg-red-100 text-red-800'
                    }`}>
                      {daysSinceNoAction === 0 ? 'Active' : `${daysSinceNoAction} days`}
                    </div>
                  </td>
...
const TagCandidateModal = ({ onClose }: { onClose: () => void }) => {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setTagFormData({ ...tagFormData, positionName: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setTagFormData({ ...tagFormData, applicant: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setTagFormData({ ...tagFormData, recruiter: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Enter recruiter name"
                />
                <p className="text-xs text-gray-500 mt-1">
                  Edit to tag your name for this position if candidate already in the system
                </p>
              </div>
              ...
              <div className="flex items-start">
                <input
                  ...
                  onChange={(e) => setTagFormData({ ...tagFormData, immediateJoiner: e.target.checked })}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setTagFormData({ ...tagFormData, status: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
 ...
 const PositionDetail = ({
  ...
  const submittedCandidates = candidates.filter(c => c.positionId === position.id && c.source === 'recruiter');
  const allDirectApplications = candidates.filter(c => c.positionId === position.id && c.source === 'direct');

  const directApplications =
    directAppStatusFilter === 'all'
      ? allDirectApplications
      : allDirectApplications.filter(c => c.status === directAppStatusFilter);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white m-6 rounded-lg">
      ...
      <div className="flex items-center mb-6">
        ...
        <h2 className="text-2xl font-bold text-gray-900">{position.positionName}</h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-3">
            ...
            {position.diversityPreference && renderField('Diversity Preference', 'diversityPreference')}
            {position.interviewMode && renderField('Interview Mode', 'interviewMode')}
            <div>
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="space-y-3">
              ...
              <div key={candidate.id} className="border rounded-lg p-4">
                <div>
                  ...
                  <div className="mt-2 text-sm text-gray-600">
                    <div>Current CTC: ₹{(candidate.currentCTC / 100000).toFixed(1)} LPA</div>
                    <div>Expected CTC: ₹{(candidate.expectedCTC / 100000).toFixed(1)} LPA</div>
            ...
            {submittedCandidates.length === 0 && (
              <p className="text-gray-500 text-center py-4">No submitted candidates yet</p>
            )}
          </div>
        </div>
            ...
            {directApplications.map(candidate => (
              <div key={candidate.id} className="border rounded-lg p-4">
                <div>
                    ...
                    {candidate.aiScore && (
                      <span
                        className={`px-2 py-1 rounded text-xs font-medium ${
                          candidate.aiScore >= 85 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}
                      >
                  ...
                  {candidate.status === 'internal_screening' && candidate.aiScore >= 85 && (
                    <div className="mt-3">
                      <button
                        onClick={() => {
                          const updatedCandidates = [...candidates];
            ...
            {directApplications.length === 0 && (
              <p className="text-gray-500 text-center py-4">
                {directAppStatusFilter === 'all'
                  ...
                  : `No candidates with "${directAppStatusFilter.replace('_', ' ')}" status`}
              </p>
            )}
          </div>
        </div>
  ...
  return (
    ...
    <div>
      <div className="max-w-6x1 mx-auto">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        {/* Header */}
        {showTagForm && <TagCandidateModal onClose={() => setShowTagForm(false)} />}
            <div className="border-b border-gray-200 p-6">
            <div className="flex justify-between items-center mb-6">
            <h2 className="text-3xl font-bold text-gray-900">Position Management</h2>
            <button 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-96 overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setNewCandidate({...newCandidate, noticePeriod: e.target.value})}
                />
              </div>
              <div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setNewCandidate({...newCandidate, engagementType: e.target.value})}
                >
              ...
              <div className="md:col-span-2">
                <label className="flex items-center">
                  <input
                    ...
                    onChange={(e) => setNewCandidate({...newCandidate, openToRelocation: e.target.checked})}
                  />
                  Open to Relocation
                </label>
              </div>
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Special Comments</label>
                <textarea
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  rows={3}
                  value={newCandidate.specialComments}
                  onChange={(e) => setNewCandidate({...newCandidate, specialComments: e.target.value})}
                />
              </div>
            </div>
      ...
      {showCreatePosition && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl m-4 max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-semibold">Create New Position</h2>
              <button
            ...
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPosition({...newPosition, clientName: e.target.value})}
                      required
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">New / Replacement Role</label>
                    <select
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.roleType}
                      onChange={(e) => setNewPosition({...newPosition, roleType: e.target.value})}
                    >
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewPosition({...newPosition, jobType: e.target.value})}
                    >
                      <option value="">Select</option>
                      <option value="Full-time">Full-time</option>
                      <option value="Part-time">Part-time</option>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewPosition({...newPosition, priority: e.target.value})}
                    >
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Client SPOC</label>
                    <input
                      type="text"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.clientSPOC}
                      onChange={(e) => setNewPosition({...newPosition, clientSPOC: e.target.value})}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Experience Needed (years)</label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPosition({...newPosition, education: e.target.value})}
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewPosition({...newPosition, function: e.target.value})}
                    >
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Select the Role</label>
                    <input
                      type="text"
                  ...
                  <div className="md:col-span-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) => setNewPosition({...newPosition, roleDescription: e.target.value})}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">CTC Budget (₹)</label>
                    <input
                      type="number"
                      placeholder="e.g., 2000000"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.ctcBudgetNumeric}
                      onChange={(e) => setNewPosition({...newPosition, ctcBudgetNumeric: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Budget for Role</label>
                    <input
                      type="text"
                      placeholder="e.g., 15-20 LPA"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.budgetForRole}
                      onChange={(e) => setNewPosition({...newPosition, budgetForRole: e.target.value})}
                    />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">CTC Structure (Fixed + Variable + ESOPs)</label>
                    <input
                      type="text"
                      placeholder="e.g., 80% Fixed + 15% Variable + 5% ESOPs"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.ctcStructure}
                      onChange={(e) => setNewPosition({...newPosition, ctcStructure: e.target.value})}
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setNewPosition({...newPosition, workplaceType: e.target.value})}
                    >
                      <option value="">Select Type</option>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPosition({...newPosition, city: e.target.value})}
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Preferred Industry (Optional)</label>
                    <input
                      type="text"
                      placeholder="e.g., Technology, SaaS"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.preferredIndustry}
                      onChange={(e) => setNewPosition({...newPosition, preferredIndustry: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Preferred Companies (Optional)</label>
                    <input
                      type="text"
                      placeholder="e.g., Google, Microsoft, Amazon"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.preferredCompanies}
                      onChange={(e) => setNewPosition({...newPosition, preferredCompanies: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Diversity Preference (Optional)</label>
                    <select
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.diversityPreference}
                      onChange={(e) => setNewPosition({...newPosition, diversityPreference: e.target.value})}
                    >
                      <option value="">No Preference</option>
                      <option value="Gender Diverse">Gender Diverse</option>
                      <option value="Ethnically Diverse">Ethnically Diverse</option>
                      <option value="Age Diverse">Age Diverse</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Target (Number of candidates to hire)</label>
                    <input
                      type="number"
                      placeholder="e.g., 2"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPosition({...newPosition, rounds: e.target.value})}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPosition({...newPosition, interviewerName: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Interviewer LinkedIn</label>
                    <input
                      type="text"
                      placeholder="e.g., linkedin.com/in/sarahjohnson"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.interviewerLinkedIn}
                      onChange={(e) => setNewPosition({...newPosition, interviewerLinkedIn: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Mode of Interview (Optional)</label>
                    <select
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      value={newPosition.interviewMode}
                      onChange={(e) => setNewPosition({...newPosition, interviewMode: e.target.value})}
                    >
                      <option value="">Select Mode</option>
                      <option value="Video Call">Video Call</option>
                      <option value="In-person">In-person</option>
                      <option value="Phone Call">Phone Call</option>
                      <option value="In-person + Video">In-person + Video</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Deadline to Close</label>
                    <input
                      type="date"
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setNewPosition({...newPosition, prompt: e.target.value})}
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Preferred Industries (Multiple allowed)</label>
                    <div className="grid grid-cols-3 gap-2">
                      {['Technology', 'Fintech', 'E-commerce', 'Healthcare', 'Education', 'Manufacturing', 'Consulting', 'Media', 'Real Estate'].map(industry => (
                        <label key={industry} className="flex items-center">
                          <input
                            type="checkbox"
                            className="mr-2"
                            checked={newPosition.aiPreferredIndustries.includes(industry)}
                            onChange={() => handleMultiSelectChange('aiPreferredIndustries', industry)}
                          />
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Preferred Functions (Multiple allowed)</label>
                    <div className="grid grid-cols-3 gap-2">
                      {['Engineering', 'Product', 'Sales', 'Marketing', 'Operations', 'Finance', 'HR', 'Design', 'Strategy'].map(func => (
                        <label key={func} className="flex items-center">
                          <input
                            type="checkbox"
                            className="mr-2"
                            checked={newPosition.aiPreferredFunctions.includes(func)}
                            onChange={() => handleMultiSelectChange('aiPreferredFunctions', func)}
                          />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewPosition({...newPosition, aiPreferredRole: e.target.value})}
                    />
                  </div>
                </div>
              </div>
            ...
            <div className="flex gap-4 mt-8 pt-6 border-t">
              <button
                ...
                disabled={!newPosition.positionName || !newPosition.clientName}
              >
                Create Position
              </button>
              <button
